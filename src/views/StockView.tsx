...
export const StockView: React.FC = () => {
  const { settings, formatCurrency, currency, refreshKey, openModal } = useApp();
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className={`p-6 sm:p-8 rounded-3xl text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 transition ${
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs sm:text-sm opacity-90 max-w-xl font-medium">
            Real-time milk balance calculation based on: <code className="bg-black/20 px-2 py-0.5 rounded font-mono text-xs">Current Stock = Total Purchased - Total Sold</code>
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-slate-900 font-bold text-xs hover:bg-slate-50 transition shadow-lg active:scale-95"
          >
            <PlusCircle className="w-4 h-4 text-emerald-600" />
            <span>+ Purchase Milk</span>
          </button>
          <button
            onClick={() => openModal('ADD_SALE')}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-black/20 hover:bg-black/30 text-white font-bold text-xs transition border border-white/20 active:scale-95"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <div className="mt-3">
            <span className="text-3xl sm:text-4xl font-black text-sky-600 dark:text-sky-400 tabular-nums">
              -{stockData.todaySale}
            </span>
            <span className="ml-1 text-sm font-semibold text-slate-400">{settings.default_unit || 'L'}</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-500 pt-2 border-t border-slate-100 dark:border-slate-800">
            Outflow to customers today
          </div>
        </div>
        ...
        <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <div className="mt-3">
            <span className="text-3xl sm:text-4xl font-black text-purple-600 dark:text-purple-400 tabular-nums">
              {stockData.todayRemaining > 0 ? `+${stockData.todayRemaining}` : stockData.todayRemaining}
            </span>
            <span className="ml-1 text-sm font-semibold text-slate-400">{settings.default_unit || 'L'}</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-500 pt-2 border-t border-slate-100 dark:border-slate-800">
            Today Purchase vs Sale balance
          </div>
        </div>
      ...
      <div className="p-5 sm:p-6 rounded-3xl bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
        <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3">
          Cumulative Milk Flow Equation
        </h3>

        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex-1 w-full">
            <span className="text-xs text-slate-400 block font-medium">Total Milk Purchased</span>
            <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400 tabular-nums">
              {stockData.totalPurchased.toFixed(1)} {settings.default_unit || 'L'}
            </span>
          </div>

          <span className="text-2xl font-black text-slate-400 shrink-0">−</span>

          <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex-1 w-full">
            <span className="text-xs text-slate-400 block font-medium">Total Milk Sold</span>
          ...
          <div className="p-4 rounded-2xl bg-emerald-500 text-white shadow-md flex-1 w-full">
            <span className="text-xs text-emerald-100 block font-bold">Current Tank Balance</span>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
              Recent Milk Stock Inflow & Outflow Movement Log
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Chronological log of transactions impacting current milk volume
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs sm:text-sm">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-400 text-[11px] uppercase tracking-wider">
                ...
                <th className="py-3 px-3">Party (Supplier / Customer)</th>
              ...
              {stockData.movements.map((m, idx) => (
                <tr key={`${m.type}-${m.id}-${idx}`} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40">
                  ...
                  <td className="py-3 px-3 text-right font-bold tabular-nums">
                    <span className={m.type === 'PURCHASE' ? 'text-emerald-600' : 'text-sky-600'}>
                      {m.type === 'PURCHASE' ? `+${m.quantity}` : `-${m.quantity}`} {m.unit}
                    </span>
                  </td>
                  <td className="py-3 px-3 text-right text-slate-600 dark:text-slate-400 tabular-nums">{currency}{m.rate}</td>

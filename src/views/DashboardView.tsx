...
export const DashboardView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-rose-600/90 hover:bg-rose-600 text-white font-bold text-xs transition shadow-lg border border-rose-400/30 active:scale-95"
            >
      ...
      {/* TODAY'S CORE METRICS ROW (6 CARDS) */}
      <div>
        <div className="flex items-center justify-between mb-3 px-1">
          <h2 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-2">
            <Calendar className="w-4 h-4 text-emerald-600" /> Today's Operations (আজকের হিসাব)
          </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
          ...
          <div 
            ...
            className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-emerald-400 dark:hover:border-emerald-600 transition cursor-pointer group"
          >
            ...
            <div className="mt-2">
              <span className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tabular-nums">
                {stats.today.purchaseQty} <span className="text-xs font-semibold text-slate-400">{settings.default_unit || 'L'}</span>
              </span>
            </div>
            <div className="mt-2 flex items-center justify-between text-[11px] pt-1.5 border-t border-slate-100 dark:border-slate-800">
          ...
          <div 
            ...
            className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-sky-400 dark:hover:border-sky-600 transition cursor-pointer group"
          >
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 text-xs font-semibold">
              <span>Today Sales</span>
              <div className="p-1.5 rounded-lg bg-sky-50 dark:bg-sky-950/60 text-sky-600 dark:text-sky-400 group-hover:scale-110 transition-transform">
                <ShoppingCart className="w-3.5 h-3.5" />
              </div>
            </div>
            <div className="mt-2">
              <span className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tabular-nums">
                {stats.today.salesQty} <span className="text-xs font-semibold text-slate-400">{settings.default_unit || 'L'}</span>
              </span>
            </div>
            <div className="mt-2 flex items-center justify-between text-[11px] pt-1.5 border-t border-slate-100 dark:border-slate-800">
          ...
          {/* Card 3: Today's Expenses (NEW!) */}
          <div 
            onClick={() => setActiveView('expenses')}
            className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-rose-400 dark:hover:border-rose-600 transition cursor-pointer group"
          >
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 text-xs font-semibold">
              <span>Today Expenses</span>
              <div className="p-1.5 rounded-lg bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 group-hover:scale-110 transition-transform">
                <Receipt className="w-3.5 h-3.5" />
              </div>
            </div>
            <div className="mt-2">
              <span className="text-xl sm:text-2xl font-black text-rose-600 dark:text-rose-400 tabular-nums">
                {formatCurrency(stats.today.expenses || 0)}
              </span>
            </div>
            <div className="mt-2 flex items-center justify-between text-[11px] pt-1.5 border-t border-slate-100 dark:border-slate-800">
              <span className="text-slate-400">Overhead:</span>
              <span className="font-bold text-rose-600 dark:text-rose-400">
                {stats.today.expenses > 0 ? 'Deducted' : '৳0 recorded'}
              </span>
            </div>
          </div>

          {/* Card 4: Today's Waste */}
          <div 
            onClick={() => setActiveView('waste')}
            className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-amber-400 dark:hover:border-amber-600 transition cursor-pointer group"
          >
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 text-xs font-semibold">
              <span>Today Waste</span>
            ...
            <div className="mt-2">
              <span className="text-xl sm:text-2xl font-black text-amber-600 dark:text-amber-400 tabular-nums">
                {(stats.today.wasteQty || 0).toFixed(1)} <span className="text-xs font-semibold text-slate-400">{settings.default_unit || 'L'}</span>
              </span>
            </div>
            <div className="mt-2 flex items-center justify-between text-[11px] pt-1.5 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            ...
            <div className="mt-2 flex items-baseline gap-1.5">
              <span className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tabular-nums">
                {stats.currentStock.toFixed(1)} <span className="text-xs font-semibold text-slate-400">{settings.default_unit || 'L'}</span>
              </span>
              <span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-950/80 px-1 py-0.2 rounded">
                Tank
              </span>
            </div>
            <div className="mt-2 flex items-center justify-between text-[11px] pt-1.5 border-t border-slate-100 dark:border-slate-800">
      ...
      {/* MONTHLY & CUMULATIVE SNAPSHOT ROW (WITH EXPENSES) */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        {/* Monthly Expenses Snapshot (NEW!) */}
        <div 
          onClick={() => setActiveView('expenses')}
          className="p-3.5 rounded-2xl bg-rose-50/70 dark:bg-rose-950/30 border border-rose-200/60 dark:border-rose-900/40 cursor-pointer hover:border-rose-400 transition"
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-rose-700 dark:text-rose-400 font-semibold block">Monthly Expenses</span>
          ...
          <span className="text-[10px] text-rose-500/80 dark:text-rose-400/80 block">Operating overheads</span>
        </div>
        ...
        <div className="p-3.5 rounded-2xl bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200/60 dark:border-emerald-900/40">
          <span className="text-[11px] text-emerald-700 dark:text-emerald-400 font-semibold block">Month Net Profit</span>
          ...
          <span className="text-[10px] text-emerald-600/80 dark:text-emerald-400/80 block">After expenses & waste</span>
        </div>

        {/* Total Cumulative Expenses (NEW!) */}
        <div 
          onClick={() => setActiveView('expenses')}
          className="p-3.5 rounded-2xl bg-slate-100/80 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60 cursor-pointer hover:border-slate-400 transition"
        >
          <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium block">All-Time Expenses</span>
          <p className="text-base font-bold text-rose-600 dark:text-rose-400 tabular-nums mt-0.5">
            {formatCurrency(stats.allTime.totalExpenses || 0)}
          </p>
          <span className="text-[10px] text-slate-400 block">Total overheads spent</span>
        </div>
      ...
      <div className="p-5 sm:p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
              Daily Milk Movement & Financial Trends (আয়, ব্যয় ও লাভ ট্রেন্ড)
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Comparative visualization of daily milk purchases, sales revenue, operating expenses, and net profit
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Chart 2: Revenue, Cost, Expenses & Profit */}
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3">
              Revenue, Expenses & Profit Trends ({currency})
            </h4>
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <Line type="monotone" dataKey="expenses" name="Expenses (ব্যয়)" stroke="#f43f5e" strokeWidth={2} dot={{ r: 3 }} />
                  <Line type="monotone" dataKey="grossProfit" name="Gross Profit" stroke="#10b981" strokeWidth={2} dot={{ r: 2 }} />
                  <Line type="monotone" dataKey="netProfit" name="Net Profit" stroke="#f59e0b" strokeWidth={2.5} strokeDasharray="4 4" dot={{ r: 3 }} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      {/* EXPENSE CATEGORIES SUMMARY WIDGET (IF ANY EXPENSES EXIST) */}
      {stats.expenseCategories && stats.expenseCategories.length > 0 && (
        <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <Receipt className="w-4 h-4 text-rose-600" /> Expense Categories Breakdown (ক্যাটাগরি ভিত্তিক খরচের হিসাব)
            </h3>
            <button
              onClick={() => setActiveView('expenses')}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {stats.expenseCategories.map(cat => (
              <div key={cat.category} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 truncate">{cat.category}</span>
                  <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-rose-100 dark:bg-rose-950/80 text-rose-700 dark:text-rose-300 font-bold">
                    {cat.count}টি
                  </span>
                </div>
                <p className="text-sm font-black text-rose-600 dark:text-rose-400 mt-1 tabular-nums">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Receipt className="w-4 h-4 text-rose-600" /> Recent Expenses (সাম্প্রতিক খরচ)
              </h3>
              <button
                onClick={() => setActiveView('expenses')}
                className="text-xs font-semibold text-rose-600 dark:text-rose-400 hover:underline"
                ...
                stats.recentExpenses.slice(0, 4).map(e => (
                  <div key={e.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400">{e.date} • <span className="font-semibold text-rose-600 dark:text-rose-400">{e.category}</span></span>
                    </div>
                    <div className="text-right">
